...
/* SERVICES — editorial */
.svc-header{margin-bottom:72px;position:relative}
.svc-header-top{display:flex;justify-content:space-between;align-items:flex-end;margin-bottom:0;gap:40px}
.svc-big-num{font-family:var(--serif);font-size:clamp(80px,10vw,140px);font-style:italic;color:rgba(255,255,255,.04);line-height:1;position:absolute;right:0;top:-20px;pointer-events:none;letter-spacing:-4px}
.svc-intro{font-size:15px;color:var(--w60);line-height:1.8;max-width:340px}
.svc-grid{display:grid;grid-template-columns:repeat(3,1fr);gap:1px;background:rgba(255,255,255,.06);border-radius:24px;overflow:hidden}
.svc-card{padding:44px 36px;background:var(--ink2);transition:all .35s;position:relative;overflow:hidden;cursor:pointer}
.svc-card::after{content:'';position:absolute;bottom:0;left:0;right:0;height:2px;background:var(--lilac);transform:scaleX(0);transition:transform .35s;transform-origin:left}
.svc-card:hover::after{transform:scaleX(1)}
.svc-card:hover{background:var(--ink3)}
.svc-num{font-family:var(--serif);font-size:11px;font-style:italic;color:var(--lilac);margin-bottom:24px;opacity:.6;display:flex;align-items:center;gap:8px}
.svc-num::after{content:'';flex:1;height:1px;background:rgba(180,127,232,.2)}
...
.svc-link{display:inline-flex;align-items:center;gap:6px;font-size:11px;font-weight:600;letter-spacing:1.5px;text-transform:uppercase;color:var(--lilac);margin-top:20px;opacity:0;transition:opacity .3s;background:none;border:none;cursor:pointer;font-family:var(--sans)}
...
.process-grid{display:grid;grid-template-columns:repeat(4,1fr);gap:0;border-top:1px solid rgba(255,255,255,.07)}
.proc-card{padding:44px 36px 44px 0;position:relative;border-right:1px solid rgba(255,255,255,.07)}
.proc-card:last-child{border-right:none}
.proc-card:first-child{padding-left:0}
.proc-num{font-family:var(--serif);font-size:clamp(80px,8vw,110px);font-style:italic;color:rgba(255,255,255,.05);line-height:1;margin-bottom:20px;letter-spacing:-3px}
.proc-title{font-family:var(--serif);font-size:22px;font-weight:400;margin-bottom:10px;letter-spacing:-.5px}
.proc-desc{font-size:13px;color:var(--w60);line-height:1.8}
.proc-tag{font-size:9px;font-weight:700;letter-spacing:2.5px;text-transform:uppercase;color:var(--lilac);margin-bottom:14px;display:block}
...
.case-visual{height:240px;position:relative;overflow:hidden;display:flex;align-items:flex-end;padding:24px}
.case-visual-bg{position:absolute;inset:0;transition:transform .6s ease}
.case-card:hover .case-visual-bg{transform:scale(1.06)}
.case-overlay{position:absolute;inset:0;background:linear-gradient(to top,rgba(10,10,10,.85) 0%,transparent 60%)}
.case-tag-vis{position:relative;z-index:1;font-size:10px;font-weight:700;letter-spacing:2px;text-transform:uppercase;color:rgba(255,255,255,.7);background:rgba(255,255,255,.1);border:1px solid rgba(255,255,255,.2);padding:4px 12px;border-radius:100px}
...
@media(max-width:960px){
  ...
  .svc-grid{grid-template-columns:1fr 1fr}.cases-grid,.test-grid,.blog-grid,.ba-grid{grid-template-columns:1fr}
...
@media(max-width:600px){
  ...
  .test-intro{grid-template-columns:1fr;gap:16px}
  ...
  return(
    <div className="pv-wrap">
      <nav className="pv-nav">
        <button className="nav-logo" onClick={onBack}>dois<span>E</span></button>
        <div style={{display:"flex",gap:10}}>
          <button className="btn-outline btn-sm" onClick={copy}><I n="cp" s={14}/>{cp?"Copiado!":"Copiar link"}</button>
          <button className="btn-outline btn-sm" onClick={onBack}>← Voltar</button>
        </div>
      </nav>
      <div className="pv-inner">
        <div className="pv-badge"><span style={{width:6,height:6,background:"var(--lilac)",borderRadius:"50%",display:"inline-block"}}/> Proposta Exclusiva</div>
        <div className="pv-to">Apresentado para</div>
        <div className="pv-client">{p.clientName||"Cliente"}</div>
        {p.headline&&<div className="pv-hl">{p.headline}</div>}
        <div style={{fontSize:13,color:"var(--w60)",display:"flex",gap:16,flexWrap:"wrap"}}>
          <span>Por <strong style={{color:"var(--lilac)"}}>doisE Agency</strong></span>
          <span>· {p.createdAt}</span>
          {p.validity&&<span>· Válido por {p.validity} dias</span>}
        </div>
        <div className="pv-div"/>
        <div className="pv-items-t">Escopo e Entregáveis</div>
        {p.items.map((it,i)=>(
          <div key={i} className="pv-item">
            <div><span className="pv-item-n">{it.d||`Item ${i+1}`}</span>{it.q>1&&<span className="pv-item-q">× {it.q}</span>}</div>
            <div className="pv-item-p">{fmt((parseFloat(it.p)||0)*(parseInt(it.q)||1))}</div>
          </div>
        ))}
        <div className="pv-total-box">
          <div>
            <div style={{fontSize:11,color:"var(--w60)",marginBottom:6,letterSpacing:.5}}>Investimento total</div>
            <div className="pv-total-v">{fmt(p.total)}</div>
          </div>
          {p.payment?.length>0&&(
            <div>
              <div style={{fontSize:11,color:"var(--w60)",marginBottom:10,letterSpacing:.5}}>Condições de pagamento</div>
              <div className="pv-chips">{p.payment.map((py,i)=><div key={i} className="pv-chip">{py.l}: {fmt(py.v)}</div>)}</div>
            </div>
          )}
        </div>
        {p.notes&&<><div className="pv-div"/><div style={{fontSize:14,color:"var(--w60)",lineHeight:1.85,fontStyle:"italic",fontFamily:"var(--serif)"}}>{p.notes}</div></>}
        ...
        <div className="pv-valid">doisE Agency · doise.com.br · Proposta gerada em {p.createdAt}</div>
      </div>
    </div>
  );
}
...
function HomePage(){
  ...
  return(
    <>
      ...
      <section className="sec" id="servicos">
        <div className="svc-header">
          ...
          <div className="svc-header-top">
            <div>
              <div className="sec-label">O que fazemos</div>
              <h2 className="sec-h2">Serviços que <em>geram resultado</em></h2>
            </div>
            <p className="svc-intro">Cada serviço da doisE foi pensado para entregar resultado real — não apenas métricas de vaidade.</p>
          </div>
        </div>
        <div className="svc-grid">
          {SVCS.map((s,i)=>(
            <div key={i} className="svc-card" onClick={()=>nav(`/?servico=${s.slug}`)}>
              <div className="svc-num">0{i+1}</div>
              <div className="svc-name">{s.n}</div>
              <div className="svc-desc">{s.d}</div>
              <button className="svc-link">Ver detalhes <I n="ar" s={12}/></button>
            </div>
          ))}
        </div>
      </section>
      ...
      {/* PROCESS */}
      <section className="sec sec-ink2" id="processo">
        <div className="process-intro">
          <div>
            <div className="sec-label">Como trabalhamos</div>
            <h2 className="sec-h2">Processo <em>transparente</em>,<br/>resultado previsível</h2>
          </div>
          <p style={{fontSize:14,color:"var(--w60)",maxWidth:300,lineHeight:1.85,paddingBottom:4}}>Do primeiro briefing ao relatório mensal — você sabe exatamente o que está acontecendo em cada etapa.</p>
        </div>
      ...
      {/* CASES */}
      <section className="sec sec-ink2" id="cases">
      ...
      <section className="sec sec-cream" style={{paddingTop:0}}>
        <div className="test-intro">
          ...
          <p style={{fontSize:14,color:"rgba(10,10,10,.55)",lineHeight:1.8}}>Histórias reais de quem escolheu crescer com estratégia.</p>
        </div>
      ...
      <section className="sec" id="contato">
        <div className="contact-wrap">
          <div>
            ...
            {[{i:"wa",l:"WhatsApp",v:"(11) 9 9999-9999"},{i:"mg",l:"E-mail",v:"oi@doise.com.br"},{i:"ig",l:"Instagram",v:"@doise.agency"}].map((c,i)=>(
      ...
      <footer className="footer">
        <div className="footer-top">
          ...
          <div>
            ...
            <div style={{fontSize:13,color:"rgba(255,255,255,.4)",marginBottom:20}}>@doise.agency</div>
